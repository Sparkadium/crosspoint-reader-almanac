CrossPoint Almanac
An offline Palm-Pilot-style PDA firmware for the Xteink X4,
built as a fork of CrossPoint Reader.
...
WatchyAlmanac, a Watchy V2 firmware:
Module	What it is
...
Upstream CrossPoint's `SCOPE.md` is explicit:
...
original copyright (see `LICENSE`). Bug fixes to reader code that
...
`partition 0 invalid magic number 0xffff`, see Flashing notes.
2. Put the data on the SD card
The Almanac reads its data from the root of the microSD card. None of these
files are in this repository (see
Data provenance for why, and how to build
them):
```
/dictionary.cdb     dictionary        (built by prepare_dict_fat.py)
/gazetteer.cdb      world factbook    (built by make_gazetteer.py)
/problems.bin       tsumego problems  (built by tsumego_pack.py)
/chess.bin          chess puzzles     (built by chess_pack.py)
```
Any module whose file is missing says so on screen and does nothing else; the
rest keep working. `chess_full.bin` is also accepted as a fallback name.
...
`docs/almanac-data-provenance.md`. No data
